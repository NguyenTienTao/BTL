...
string chooseWord();
char readAGuess(string& guessedWord, int badGuessesCount);
...
int main()
{
	...
	string badGuesses = "";
    int round = 0, badGuessCount = 0;

	do {
		renderGame(guessedWord, badGuesses, round, badGuessCount, word);
		char guess = readAGuess(guessedWord, badGuessCount);
	...
	} while (badGuessCount < MAXBADGUESSES && word != guessedWord);

	renderGame(guessedWord, badGuesses, round, badGuessCount, word);
	if (badGuessCount < MAXBADGUESSES)
		cout << "Good job! You win!";
	else
		cout << "You lose. The correct word is " << word;
...
char readAGuess(string& guessedWord, int badGuessCount)
{
	if (badGuessCount < MAXBADGUESSES) {
        char ch;
        cout << "Your Guess: ";
        cin >> ch;
        while(guessedWord.find_first_of(ch) != string::npos) {
            cout << "Again your guess: ";
            cin >> ch;
        }
        return ch;
	}
